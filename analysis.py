...
def graphTimeComplexity():
  ...
  for i in range(len(matrix_sizes)):
    ...
    linear_time = timeit.timeit(stmt=f'multiplyMatrices("linear", {size})', setup='from __main__ import multiplyMatrices', number=repeat) / repeat
    linear_times.append(linear_time)
    print('linear', linear_time)

    dnc_time = timeit.timeit(stmt=f'multiplyMatrices("dnc", {size})', setup='from __main__ import multiplyMatrices', number=repeat) / repeat
    dnc_times.append(dnc_time)
    print('dnc', dnc_time)

    strassen_time = timeit.timeit(stmt=f'multiplyMatrices("strassen", {size})', setup='from __main__ import multiplyMatrices', number=repeat) / repeat
